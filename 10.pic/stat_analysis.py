...
def print_mean(df, metric):
    ...
    """
    mean = df.groupby(['Features'], as_index=False).agg({metric: ['mean', 'std', 'max', 'min']})
    ...
    for dataset in DATASETS:
        for metric in metrics:
            for model in models:
                ...
                k = kruskal_wallis(df, metric, 'Features', sort=False)
                ...
                plt.savefig("results/resultados-tese-features" + metric + "-" + model + ".pdf", bbox_inches='tight')
                plt.cla()
                plt.close(fig)
