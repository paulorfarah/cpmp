...
def print_mean(df, metric):
    ...
    """
    mean = df.groupby([metric], as_index=False).agg({'ROC': ['mean', 'std', 'max', 'min']})
...
def rq1(dataset, feature_sets, metric):
    # datasets = ['all_results_trad_slw.csv']
    # feature_sets = ['Str', 'StrEvo', 'Evo']
    # models = ['Evo']
    Path("results/stats").mkdir(parents=True, exist_ok=True)
    ...
    k = kruskal_wallis(df, 'ROC', metric)

    kruskal_result, posthoc = k.apply(ax)
    plt.tight_layout()

    # kruskal_results
    ax.set_ylabel("ROC/AUC - " + metric + " analysis")
    plt.savefig("results/stats/results-methods-" + metric + "-all.pdf", bbox_inches='tight')
    plt.cla()
    plt.close(fig)
    mean, best = print_mean(df, metric)
    for fs in feature_sets:
        print(fs)
        dfm = df.drop(df[df.model != fs].index)
        # dfm = dfm.sort_values('window')
        dfm = dfm.sort_values(metric)
        fig, ax = plt.subplots(figsize=(10, 6))
        # for KRUSKAL WALLIS ANALYSIS
        # k = kruskal_wallis(dfm, 'ROC', 'window')
        k = kruskal_wallis(dfm, 'ROC', metric)
        kruskal_result, posthoc = k.apply(ax)
        plt.tight_layout()

        # kruskal_results
        ax.set_ylabel("ROC/AUC - " + metric + " analysis " + fs)
        plt.savefig("results/stats/results-methods-" + metric + "-AUC-" + fs + ".pdf", bbox_inches='tight')
        plt.cla()
        plt.close(fig)
        mean, best = print_mean(dfm, metric)
        ...
        try:
            ...
            print("No statistical significance" + fs)
