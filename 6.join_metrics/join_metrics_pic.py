...
if __name__ == "__main__":
    ...
    for project_name in projects:
        ...
        csv_results = 'results/' + project_name + '-pic-all-releases-' + merge_type + '.csv'
                        ...
                        if len(evo.index) and len(df_joined.index):
                            ...
                            if len(change_distiller.index) and len(df_joined.index):
                                ...
                                if len(df_joined.index):
                                    ...
                                    df_joined['will_change_median'] =np.where(df_joined['TOTAL_CHANGES'] > medianChanges, 1, 0)
                                    print(df_joined['will_change_median'].value_counts())
                                    df_joined['will_change'] = np.where(df_joined['TOTAL_CHANGES'] > 0, 1, 0)
                                    print(df_joined['will_change'].value_counts())
                                    if release == 1:
